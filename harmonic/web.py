...
<table><tr><th>標的</th><th>週期</th><th>型態</th><th>方向</th><th>狀態</th>
<th>進場</th><th>止損</th><th>TP1/2/3</th><th>品質</th><th>R</th></tr>
{% for r in rows %}
<tr><td>{{r.symbol}}</td><td>{{r.timeframe}}</td><td>{{r.pattern}}</td>
...
def create_app(cfg: Config) -> Flask:
    ...
    def _filtered():
        ...
        return sigs, days, fstatus

    @app.route("/")
    def index():
        sigs, days, fstatus = _filtered()
        return render_template_string(
            _PAGE, s=summarize(sigs),
            rows=[s.to_row() for s in sigs[:300]],
            days=days, fstatus=fstatus,
        )

    @app.route("/api/stats")
    def api_stats():
        sigs, _, _ = _filtered()
        return jsonify(summarize(sigs))
